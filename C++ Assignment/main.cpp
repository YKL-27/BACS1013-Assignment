...
void cancelBooking(string username, string currentPassword, bookingType* bookingsArray, int lenBookings, int* bookingIndices, int bookingCount) {
    ...
    for (int i = 0; i < bookingCount; i++) {
        ...
        cout << "\t\t" << setw(4) << left << i + 1
            << setw(15) << left << formatDate(bookingsArray[idx].day);

        if (bookingsArray[idx].service == 3) {
            // Styling Consultation (1-hour slots)
            cout << setw(25) << left << TIMESLOT_CONSULT[bookingsArray[idx].timeslot];
        }
        else {
            // Other services (2-hour slots)
            cout << setw(25) << left << TREATMENT_TIMESLOT[bookingsArray[idx].timeslot / 2];
        }

        cout << setw(20) << left << EXPERT[bookingsArray[idx].expert]
...
void viewReceipt(string username, bookingType* bookingsArray, int* bookingIndices, int bookingCount) {
    ...
    // Use formatDate to display the date properly
    ...
    if (booking.service == 3) {  // Consultation (1-hour slots)
        ...
        cout << "\t\t\t\t\tTime\t\t\t: " << TREATMENT_TIMESLOT[booking.timeslot / 2] << endl; // Use timeslot / 2 for other services
    }

    cout << "\t\t\t\t\tExpert\t\t\t: " << EXPERT[booking.expert] << endl;
    cout << "\t\t\t\t\tService\t\t\t: " << SERVICE[booking.service] << endl;

    // Use booking.cost instead of COST[booking.service] to match mybookingsDetail
    cout << "\t\t\t\t\tCost\t\t\t: RM " << fixed << setprecision(2) << booking.cost << endl;
...
void generateReceipt(const bookingType& booking) {
    ...
    if (booking.service == 3) {  // Styling Consultation (1-hour slots)
        ...
        receiptFile << "Time\t\t\t: " << TREATMENT_TIMESLOT[booking.timeslot / 2] << endl;  // Ensure timeslot/2 for non-consultation services
    }

    receiptFile << "Expert\t\t\t: " << EXPERT[booking.expert] << endl;
    receiptFile << "Service\t\t\t: " << SERVICE[booking.service] << endl;  // Use SERVICE array for consistency
    receiptFile << "Cost\t\t\t: RM " << fixed << setprecision(2) << booking.cost << endl;  // Use booking.cost instead of servicePrices
